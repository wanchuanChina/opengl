...
public abstract class AFilter {
    ...
    public int getInt(int type, int index) {
        ...
        int[] ints = mInts.get(type);
        if (ints == null || ints.length <= index){
            return 0;
        }
        return ints[index];
    }
    ...
    protected void onBindTexture(){
        GLES30.glActiveTexture(GLES30.GL_TEXTURE0+textureType); // 设置激活的纹理单元,默认使用2D
        GLES30.glBindTexture(GLES30.GL_TEXTURE_2D,getTextureId()); // 绑定纹理
        GLES30.glUniform1i(mHTexture,textureType); // 纹理采样器设置
    }
